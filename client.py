# code imported from this repo: https://github.com/theailanguage/mcp_client/
...
from typing import (
    Optional,
)  # For type annotations, e.g., indicating that a variable may be None
...
class MCPClient:
    ...
    async def process_query(self, query: str) -> str:
        ...
        for candidate in response.candidates:
            ...
            ):  # Ensure that the response has parts (sections).
...
def clean_schema(schema):
    """
    Recursively remove unsupported fields like 'title' and 'additionalProperties' from a JSON schema.
    """
    if isinstance(schema, dict):
        schema.pop("title", None)
        schema.pop(
            "additionalProperties", None
        )  # <--- Remove this line if you want to allow it
